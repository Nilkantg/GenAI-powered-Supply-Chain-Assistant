...
class Model(BaseModel):
    ...
    tools: List[object] = Field(default_factory=tool_binding, description="List of tools to be used by the agent")
...
def build_model(model: Model) -> dict:
    ...
    if model.vector_store is None:
        persist_directory = "./chroma_db"
        embedding_model = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
        model.vector_store = Chroma(
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        1. Parse the query to identify the SKU (e.g., 'SKU01') and the task (retrieve data, calculate EOQ, calculate reorder point, or generate report).
        ...
        - Use RetrieveData for queries about stock levels, demand, or other fields (e.g., 'get stock for SKU01').
        - Use CalculateEOQ for EOQ calculations (e.g., 'EOQ for SKU01').
        - Use CalculateReorderPoint for reorder point calculations (e.g., 'reorder point for SKU01').
        - Use GenerateReport for comprehensive reports (e.g., 'report for SKU01').
